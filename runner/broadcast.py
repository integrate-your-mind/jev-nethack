...
class BroadcastRecorder:
    ...
    def decision(self, *, episode: int, step: int, decision: Mapping[str, Any]) -> None:
    ...
    def _finish_segment(self, index: int, frames: list[tuple[Path, float]], action_count: int,
        ...
        try:
            ...
            (self.output / f"segment-{index:04d}.manifest.json").write_bytes(manifest_bytes)
    ...
    def finalize(self, *, reason: str) -> dict[str, Any]:
        ...
        (self.output / "manifest.receipt.json").write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n")
        return manifest
